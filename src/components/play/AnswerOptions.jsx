...
export default function AnswerOptions({ question, onAnswer, disabled, removedOptionIds = [] }) {
  const [selected, setSelected] = useState(null)
  const options = question?.question_options ?? []

  function handleSelect(opt) {
    if (disabled || selected || removedOptionIds.includes(opt.id)) return
    setSelected(opt.id)
        ...
        {options.map((opt, i) => {
          ...
          return (
            <button
              ...
              disabled={disabled || !!selected || isRemoved}
              className={`rounded-xl px-4 py-6 text-left font-semibold text-lg transition-all border-2
                ${isRemoved ? 'opacity-30 border-gray-700 bg-gray-800 text-gray-500 cursor-not-allowed' :
                  selected === opt.id
